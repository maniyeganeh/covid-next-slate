...
const WorldVaccineComponent = ({ data }) => {
  const { dark } = useContext(CovidContext);
  const population = 7856616647;
  const fullVac = data.map((dt) => dt.secondDose);

  const vaccinePercent = parseFloat((fullVac[0] / population) * 100).toFixed(2);
  let darkBg;
  if (dark) {
    darkBg = "#4a69bd";
  } else {
  ...
  return (
    <div className={`${classes.worldContainer} rtl`}>
      <div className={classes.worldHeader}>
        <div
          className={
            !dark
              ? classes.headerBox
              : `${classes.headerBox} ${classes.headerBoxDark}`
          }
        >
          آمار میزان واکسیناسیون در جهان
        </div>
      ...
      <div className={classes.world}>
        {data.map((dt, index) => (
          <>
            <div className={classes.worldRow} key={index}>
              <div
                className={
                  !dark
                    ? classes.worldBox
                    : `${classes.worldBox} ${classes.worldBoxDark}`
                }
              >
                <div className={classes.title}>مجموع واکسن تزریق شده</div>
                <h6>
                  <CountUp
                    key={dt.fistDose}
                    start={0}
              ...
              >
                <div className={classes.title}>
                  افراد واکسینه شده (تزریق دو دوز)
                </div>
                <h6>
                  <CountUp
                    key={dt.secondDose}
              ...
              >
                <div className={classes.title}>
                  نسبت افراد واکسینه شده به جمعیت
                </div>
                <h6>{vaccinePercent}%</h6>
              </div>
            </div>
            <div className={classes.date}>
              <div
                className={
                  !dark ? classes.text : `${classes.text} ${classes.textDark}`
                }
              >
                {new Date(dt?.updatedAt).toLocaleString("fa-ir")}
              </div>
            </div>
          </>
        ))}
        ...
        <Row>
          <Col xs="12" sm="12" xs="12">
            <div className={classes.progress}>
              <CircularProgressbarWithChildren
                value={vaccinePercent}
                text={`${vaccinePercent?.toLocaleString("fa-ir")}%`}
                strokeWidth={5}
                styles={buildStyles({
                  textColor: vaccinePercent > 75 ? "#2c8558" : "#c23616",
                  pathColor: vaccinePercent > 75 ? "#2c8558" : "#c23616",
                  trailColor: "#d2dae2",
                  pathTransitionDuration: 2,
                  textSize: "14px",
                })}
              ></CircularProgressbarWithChildren>
              <h3 className={dark && "text-light"}>
                نسبت افراد واکسینه شده به جمعیت
              </h3>
            </div>
          </Col>
...
export default WorldVaccineComponent;
